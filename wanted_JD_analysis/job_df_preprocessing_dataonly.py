...
job_df = pd.read_csv('C:/Users/USER/Desktop/personal_project/wanted_JD_analysis/wanted_crawling_dataonly.csv',
              encoding = 'utf-8-sig')
...
# 이번에는 직접 데이터를 가져왔으므로, 순서대로 담겨있기에 가져온다.
# 이게 하나의 공고를 여러 항목에 걸어서 여러 곳에 걸리는 것을 발견했음
# 따라서 이번에는 25개를 뽑아온 곳 기준으로 분류할 것이다.
job_df['category'] = ['MLE']*25 + ['DE']*25 + ['DS']*25 + ['DA']*25
# 마지막으로 필요없는 항목에 대한 제거 실시
job_df = job_df[['company_name', 'category', 
...
job_df_final = job_df[['company_name','category', 
                      'job_req_noun', 'job_add_noun', 'job_due_noun', 'title']]
job_df_final.to_csv('C:/Users/USER/Desktop/personal_project/wanted_JD_analysis/wanted_data_prep_dataonly.csv',
